...
class TimeAnalysis:
    ...
    def date_prep(self):
        ...
        time_series_daily = pd.DataFrame({'Date': daily_headlines.index, 'Headlines_Count': daily_headlines.values})
        time_series_daily['Date'] = pd.to_datetime(time_series_daily['Date'])
        time_series_daily.set_index('Date', inplace=True)

        time_series_monthly = pd.DataFrame({'Date': monthly_headlines.index, 'Headlines_Count': monthly_headlines.values})
        time_series_monthly['Date'] = time_series_monthly['Date'].dt.strftime('%Y-%m-%d')
        time_series_monthly['Date'] = pd.to_datetime(time_series_monthly['Date'])
        time_series_monthly.set_index('Date', inplace=True)

        time_series_yearly = pd.DataFrame({'Date': yearly_headlines.index, 'Headlines_Count': yearly_headlines.values})
    ...
    def time_decompose(self, ts_data, period, day=False):
        ...
        if day:
            decomposition = seasonal_decompose(ts_data['Headlines_Count'].loc[ts_data.index.year == 2018], model='additive', period=period)
            axes[0].plot(ts_data['Headlines_Count'].loc[ts_data.index.year == 2018], label='Original', color='blue')
        else:
            decomposition = seasonal_decompose(ts_data['Headlines_Count'], model='additive', period=period)
